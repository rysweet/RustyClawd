...
pub struct WebFetchToolPhase2 {
    cache: Arc<WebFetchCache>,
}

impl WebFetchToolPhase2 {
...
impl Default for WebFetchToolPhase2 {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl crate::Tool for WebFetchToolPhase2 {
...
mod tests {
    ...
    fn test_http_to_https_upgrade() {
        assert_eq!(
            WebFetchToolPhase2::upgrade_to_https("http://example.com"),
            "https://example.com"
        );
        assert_eq!(
            WebFetchToolPhase2::upgrade_to_https("https://example.com"),
            "https://example.com"
        );
    }
    ...
    fn test_domain_extraction() {
        assert_eq!(
            WebFetchToolPhase2::get_domain("https://docs.anthropic.com/path"),
            Some("docs.anthropic.com".to_string())
        );
        assert_eq!(
            WebFetchToolPhase2::get_domain("http://example.com:8080/path"),
    ...
    fn test_same_domain_check() {
        assert!(WebFetchToolPhase2::is_same_domain(
    ...
    fn test_markdown_conversion_html() {
        ...
        let result = WebFetchToolPhase2::to_markdown(html, Some("text/html"));
        assert!(result.is_ok());
    ...
    fn test_markdown_conversion_text() {
        ...
        let result = WebFetchToolPhase2::to_markdown(text, Some("text/plain"));
        assert!(result.is_ok());
    ...
    fn test_markdown_truncation() {
        ...
        let result = WebFetchToolPhase2::to_markdown(long_text.as_bytes(), Some("text/plain"));
        assert!(result.is_ok());
        let markdown = result.unwrap();
    ...
    async fn test_webfetch_phase2_basic() {
        let tool = WebFetchToolPhase2::new();
