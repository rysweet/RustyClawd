...
// Re-export types that were moved to sub-modules
pub use super::focus_manager::LayoutCache;
pub use super::scroll_controller::ScrollController;
...
mod tests {
    ...
    fn test_unicode_input() {
        let mut app = App::new(PermissionMode::default());
        app.insert_char('\u{1F980}');
        app.insert_char('\u{1F680}');

        assert_eq!(app.input().chars().count(), 2);
        app.move_cursor_left();
        app.insert_char('\u{2764}');
        assert_eq!(app.input(), "\u{1F980}\u{2764}\u{1F680}");
    }
