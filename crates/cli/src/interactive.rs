    ...
    pub async fn with_hooks(hooks: Option<Arc<hooks::HooksSystem>>) -> Result<Self> {
        ...
        tui.set_completion_callback(Box::new(move |prefix| {
            ...
            let built_in_commands = vec![
                ...
                ("version", Some("Show version information".to_string())),
                ("permissions", Some("Open permissions modal".to_string())),
                ("login", Some("Check authentication status".to_string())),
                ("bug", Some("Report a bug via GitHub".to_string())),
                (
                    "add-dir",
                    Some("<path> - Add working directory".to_string()),
                ),
                ("fast", Some("Toggle fast mode".to_string())),
