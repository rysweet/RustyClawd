...
impl TestConfig {
    fn from_args() -> Self {
        ...
        let cleanup_builds = args.iter().any(|a| a == "--cleanup");
        
        let cleanup_interval = args.iter()
            .position(|a| a == "--cleanup-interval")
            .and_then(|i| args.get(i + 1))
            .and_then(|s| s.parse().ok())
            .unwrap_or(5);
...
impl ContinuousTester {
    ...
    fn run(&mut self) {
        ...
        println!("Delay: {:?}", self.config.delay_between_runs);
        
        if self.config.cleanup_builds {
            println!("Cleanup: {} (every {} iterations)", 
                     "enabled".green(), 
                     self.config.cleanup_interval);
        } else {
            println!("Cleanup: {}", "disabled".dimmed());
        }
        ...
        loop {
            ...
            if self.config.cleanup_builds && iteration > 1 && (iteration - 1) % self.config.cleanup_interval == 0 {
                self.cleanup_builds();
            }
    ...
    fn cleanup_builds(&self) {
        println!("{} Running cleanup...", "🧹".yellow());
        
        // Find all RustyClawd target directories
        let target_dirs = vec![
            PathBuf::from("/home/azureuser/src/RustyClawd/target"),
            PathBuf::from(std::env::var("HOME").unwrap_or_default()).join("src/RustyClawd/target"),
        ];
        
        let mut total_freed_mb = 0u64;
        ...
        for target_dir in &target_dirs {
            ...
            // Clean up incrementals, debug artifacts, but preserve release builds
            let dirs_to_clean = vec![
                target_dir.join("debug/incremental"),
                target_dir.join("debug/deps/.fingerprint"),
            ...
            for dir in dirs_to_clean {
                if dir.exists() {
                    if let Err(e) = fs::remove_dir_all(&dir) {
                        println!("  {} Failed to remove {}: {}", "⚠".yellow(), dir.display(), e);
                    } else {
            ...
            let freed_mb = (size_before.saturating_sub(size_after)) / (1024 * 1024);
            total_freed_mb += freed_mb;
            
            if freed_mb > 0 {
                println!("  {} Freed {} MB from {}", 
                         "💾".cyan(), freed_mb, target_dir.display());
            }
        }
        
        if total_freed_mb > 0 {
            println!("{} Total freed: {} MB\n", "✨".green(), total_freed_mb);
        } else {
            println!("{} No space to free (already clean)\n", "✓".green());
        }
    }
...
fn print_usage() {
    ...
    println!("  --cleanup            Enable cleanup of target/ directories between runs");
    println!("  --cleanup-interval N Clean up every N iterations (default: 5)");
    println!("  --help               Show this help message");
    println!();
    println!("Examples:");
    println!("  ./continuous_tester --max-iterations 5 --delay 60");
    println!("  ./continuous_tester --cleanup --cleanup-interval 3");
    println!("  ./continuous_tester --max-iterations 10 --cleanup --delay 300");
}
