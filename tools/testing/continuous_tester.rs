...
impl ContinuousTester {
    ...
    fn run(&mut self) {
        ...
        loop {
            ...
            if self.config.cleanup_builds
                && iteration > 1
                && (iteration - 1) % self.config.cleanup_interval == 0
            {
                self.cleanup_builds();
            }
    ...
    fn run_test_iteration(&self, iteration: usize) -> TestResult {
        ...
        let output_file = self
            .config
            .output_dir
            .join(format!("run_{:04}_{}.log", iteration, timestamp));
        ...
        let mut child = match Command::new(&self.config.rusty_binary)
            ...
            .arg("cli")
            .arg("--max-depth")
            .arg("1") // Prevent infinite recursion
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
